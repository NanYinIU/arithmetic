package com.leetcode.v2.No026_remove_duplicate_from_sorted_array;
 ...
 * == expectedNums[i]; } 如果所有断言都通过，那么您的题解将被 通过。
 *
 * 示例 1：
 *
 * 输入：nums = [1,1,2] 输出：2, nums = [1,2,_] 解释：函数应该返回新的长度 2 ，并且原数组 nums 的前两个元素被修改为
 * 1, 2 。不需要考虑数组中超出新长度后面的元素。 示例 2：
 *
...
class Solution {
    public int removeDuplicates(int[] nums) {
        ...
        int slow_point = 0;
        int fast_point = 0;
        while (fast_point < nums.length) {
            // fast 向前移动，发现和 slow 不一样，则 slow 指向 fast
